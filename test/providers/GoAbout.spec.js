...
describe('GoAboutService', () => {
  ...
  describe('request', () => {
    beforeEach(() => {
      ...
      sandbox.stub(t.GoAbout, 'getApi').resolves(halson(t.goAboutRootApi))
  ...
  describe('checkTokenAndReturnUser', () => {
    beforeEach(() => {
      t.GoAbout.getUser = sandbox.stub()
    })

    it('should make request to GoAbout and return GoAbout user', function* () {
      t.GoAbout.getUser.resolves(halson(t.goAboutRootApi._embedded['http://rels.goabout.com/authenticated-user']))

      const result = yield t.GoAbout.checkTokenAndReturnUser(t.token)

      assert.equal(result.email, t.email)
    })

    it('should reject if getting user failed is invalid', function* () {
      t.GoAbout.getUser.rejects(new t.Errors.PassThrough(403))

      try {
        yield t.GoAbout.checkTokenAndReturnUser(t.token)
        assert.equal(false, true) // Never called
      } catch (error) {
        t.error = error
      }
